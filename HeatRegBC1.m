function varargout = HeatRegBC1(varargin)
% HEATREGBC1 MATLAB code for HeatRegBC1.fig
%      HEATREGBC1, by itself, creates a new HEATREGBC1 or raises the existing
%      singleton*.
%
%      H = HEATREGBC1 returns the handle to a new HEATREGBC1 or the handle to
%      the existing singleton*.
%
%      HEATREGBC1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HEATREGBC1.M with the given input arguments.
%
%      HEATREGBC1('Property','Value',...) creates a new HEATREGBC1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HeatRegBC1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HeatRegBC1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HeatRegBC1

% Last Modified by GUIDE v2.5 03-Aug-2016 15:50:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HeatRegBC1_OpeningFcn, ...
                   'gui_OutputFcn',  @HeatRegBC1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% UIWAIT makes HeatRegBC1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = HeatRegBC1_OutputFcn(hObject, eventdata, handles) 
...
%% OPENING FUNCTION
% * Executes just before |HeatRegBC1| is made visible;
...
function HeatRegBC1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HeatRegBC1 (see VARARGIN)

%%
% Creates the |handles| structure
% Choose default command line output for |HeatRegBC1|
handles.output = hObject;

%%
% Loading the mesh information.
load('HeatStructDef','edgesArray','L','B','Nx','Ny');
...
if exist('./HeatRegBC1.mat','file') % reuse the previous data
    load('HeatRegBC1','data');
    handles.data=data;
    %%
...
if exist('./HeatRegBC1.mat','file')
    ...
    ChangesQ = any(~isfield(comp_struct(load('HeatRegBC1.mat'),...
