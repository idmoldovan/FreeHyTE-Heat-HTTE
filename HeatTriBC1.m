function varargout = HeatTriBC1(varargin)
% HEATTRIBC1 MATLAB code for HeatTriBC1.fig
%      HEATTRIBC1, by itself, creates a new HEATTRIBC1 or raises the existing
%      singleton*.
%
%      H = HEATTRIBC1 returns the handle to a new HEATTRIBC1 or the handle to
%      the existing singleton*.
%
%      HEATTRIBC1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HEATTRIBC1.M with the given input arguments.
%
%      HEATTRIBC1('Property','Value',...) creates a new HEATTRIBC1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HeatTriBC1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HeatTriBC1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HeatTriBC1

% Last Modified by GUIDE v2.5 18-Jul-2017 16:04:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HeatTriBC1_OpeningFcn, ...
                   'gui_OutputFcn',  @HeatTriBC1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% UIWAIT makes HeatTriBC1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = HeatTriBC1_OutputFcn(hObject, eventdata, handles) 
...
%% OPENING FUNCTION
% * Executes just before |HeatTriBC1| is made visible;
...
function HeatTriBC1_OpeningFcn(hObject, eventdata, handles,varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HeatTriBC1 (see VARARGIN)

%%
% Creates the |handles| structure
% Choose default command line output for |HeatTriBC1|
...
%%
% Calls and external function that creates all topological matrices.
[nodes, edges_nodes, edges_loops, loops_nodes, loops_edges] = CreateEdgeLoop(p,t);

%%
% Getting the exterior edges. 
...
if exist('./HeatTriBC1.mat','file') % reuse the previous data
    load('HeatTriBC1','data');
    handles.data=data;
%%
...
X = zeros(nnel,nel) ; Y = zeros(nnel,nel) ;
...
patch(X,Y,'w'); axis([limxmin-0.01*abs(limxmin) limxmax+0.01*abs(limxmax) limymin-0.01*abs(limymin) limymax+0.01*abs(limymax)]); 
axis equal; 
axis off ;
