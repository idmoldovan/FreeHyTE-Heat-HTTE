function varargout = HeatRegBC2(varargin)
% HEATREGBC2 MATLAB code for HeatRegBC2.fig
%      HEATREGBC2, by itself, creates a new HEATREGBC2 or raises the existing
%      singleton*.
%
%      H = HEATREGBC2 returns the handle to a new HEATREGBC2 or the handle to
%      the existing singleton*.
%
%      HEATREGBC2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HEATREGBC2.M with the given input arguments.
%
%      HEATREGBC2('Property','Value',...) creates a new HEATREGBC2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HeatRegBC2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HeatRegBC2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HeatRegBC2

% Last Modified by GUIDE v2.5 04-Aug-2016 00:02:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @HeatRegBC2_OpeningFcn, ...
    'gui_OutputFcn',  @HeatRegBC2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% UIWAIT makes HeatRegBC2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = HeatRegBC2_OutputFcn(hObject, eventdata, handles) 
...
%% OPENING FUNCTION
% * Executes just before |HeatRegBC2| is made visible;
...
function HeatRegBC2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HeatRegBC2 (see VARARGIN)

%%
% Creates the |handles| structure
% Choose default command line output for |HeatRegBC2|
handles.output = hObject;

%%
% Loads the exterior edges and their types from the previous GUI
load('HeatRegBC1','edgesArray','data','DorN');
...
% created from from scratch or imported from the |HeatRegBC2| file, if 
...
if isfield(handles,'edgesDirichlet')==1
    ...
    if exist('./HeatRegBC2.mat','file') % reuse the previous data
        load('HeatRegBC2','dataTemp');
    ...
    uitable('units','Normalized','Position',[0.48,0.16,0.11,0.45],...
...
% created from from scratch or imported from the |HeatRegBC2| file, if such 
...
if isfield(handles,'edgesNeumann')==1
    ...
    uitable('units','Normalized','Position',[0.835,0.16,0.11,0.45],...
...
% If HeatRegBC2.mat does not exist, it creates it
if ~exist('./HeatRegBC2.mat','file')
    dummy = 0;
