...
% COMPUTEFIELDSREG computes the final temperature and flux fields, and
% stores them in the Loops structure and in an output file, if requested
% by the user.
%
% COMPUTEFIELDSREG is called by MAINREG. 
% Input:
% It receives structures Edges and  Loops, the solution vector X 
% (calculated in MAINREG), the node list Nodes and the iteration number.  
% Besides the input arguments, COMPUTEFIELDSREG loads from HeatStructDef.mat
...
% (NGP+1)^2 plotting points, in Loop structure returned to MAINREG, 
...
% The output points are, by default, the Gauss points used for the
% integration. 
pts = abscissa';
% However, other distributions are acceptable. For instance, the syntax:
% pts = linspace(-1,1,SOME_NO_OF_POINTS);
% generates SOME_NO_OF_POINTS uniformly spaced output points.


%% ************ BEGIN ITERATION ON THE ELEMENTS *************
...
for ii = 1:length(Loops.area)
    ...
    % most far apart points of the element in x and y directions.
